...
class SocEnv(gym.Env):
    ...
    def __init__(
        ...
        # PATH MANAGER
        self.path_manager = PathManager(image_path="Final_WR_World/WR_World.png", chunk_size=10)

        # RL PARAMETERS
        self.seed(seed)

        mlp_zeros = np.concatenate([np.zeros(2), np.zeros(4), np.zeros(2), np.zeros([10,2]), np.zeros(1)])
        ...
        self.observation_space = spaces.Dict(
            {
                "vector": spaces.Box(
                    low=-np.inf,
                    high=np.inf,
                    shape=(self.mlp_context_frame_length, 19),
                    dtype=np.float32,
                ),
                "image": spaces.Box(
                    low=0,
    ...
    def reset(self):
        ...
        self.path_manager.plan_new_path(self.bot.rl_bot.start_pose, self.bot.rl_bot.goal_pose)
        self.path_manager.get_next_chunk(self.bot.rl_bot.start_pose)    
    ...
    def get_observations(self):
        ...
        chunk = self.path_manager.get_next_chunk(bot_world_pos)
        mlp_states_un = [bot_world_pos[:2], bot_world_ori, goal_world_pos[:2], chunk, timestep]
        if self.state_normalize == True:
            mlp_states = [
                ...
                [chunk[i]/self.world_limits for i in range(len(chunk))],
    ...
    def step(
        ...
        reward, to_point_rew, reward_dict, ep_rew_dict = (
            self.reward_manager.compute_reward(
                prev_bot_pos[:2], mlp_obs[0], mlp_obs[-3], mlp_obs[-2], lidar_data
            )
        )
...
class CustomCombinedExtractor(BaseFeaturesExtractor):
    def __init__(
        ...
        self.vec_lstm = nn.LSTM(19, self.vec_lstm_out_size, num_layers=1)
