...
class SocialReward:
    def __init__(self, log_dir):
        ...
        self.path_corridor_width = 2.0  # Acceptable deviation width
        self.max_path_deviation = 3.0   # Maximum deviation before heavy penalties
    ...
    def compute_reward(self, prev_bot_pos, cur_bot_pos, goal_pos, path_chunk=None, lidar_data=None):
        ...
        if self.check_goal_reached(cur_bot_pos, goal_pos):
            ...
            reward_dict['path_rew'] = self.path_following_reward(cur_bot_pos, path_chunk)

            self.current_rew = (reward_dict['cosine_sim_rew'] +
                                reward_dict['dist_to_goal_rew'] +
                                reward_dict['smooth_crash_rew'] +
                                reward_dict['close_coll_rew'] +
                                reward_dict['path_rew'] +
                                self.live_reward)
    ...
    def path_following_reward(self, cur_bot_pos, path_chunk):
        ...
        for i in range(len(path_chunk) - 1):
            ...
            t = max(0, np.dot(cur_bot_pos - p1, segment) / (len_segment * len_segment))
