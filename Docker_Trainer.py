...
class SocEnv(gym.Env):
    ...
    def __init__(
        ...
        seed=int(np.random.randint(low=0, high=64, size=1)[0]),
    ) -> None:
        ...
        self.bot_min_max = np.array([-7.5, -5.5, 7.6, 5.6])  # (min_x, min_y, max_x, max_y)
        ...
        # TESTING INITS
        self.csv_dir = f"{self.logdir}/States"
        if not os.path.exists(self.csv_dir):
            os.makedirs(self.csv_dir, exist_ok=True)
        self.csv_file = f"{self.csv_dir}/Obs_df_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"

        self.obs_df = pd.DataFrame(columns=[
                                            "Episode",
                                            "Ep_Timestep",
                                            "Global_Timestep",
                                            "Bot_x", 
        ...
        self.obs_df.to_csv(self.csv_file, mode='w', header=True, index=False)
    ...
    def reset(self):
        ...
        # print(f"Start pose : {self.bot.rl_bot.start_pose[0]}")

        if self.bot.rl_bot.start_pose[0][0] < self.bot_min_max[0] or self.bot.rl_bot.start_pose[0][0] > self.bot_min_max[2] or self.bot.rl_bot.start_pose[0][1] < self.bot_min_max[1] or self.bot.rl_bot.start_pose[0][1] > self.bot_min_max[3]:
            print("Junk Start Created! Respawning...")
            self.episode_count -= 1
            self.reset()
        ...
        for pts in self.path_manager.current_path:
            print(pts)

        # self.obs_df.to_csv(self.csv_file, mode='a', header=False, index=False)

        observations, _, _, _, _ = self.get_observations()

        self.logger.info(f"Episode {self.episode_count} started")
        wandb.log({"episode": self.episode_count, "seed": self.seed})
        return observations
    ...
    def _get_vector_observation(self):
        ...
        mlp_states = [
            ...
            self.world_min_max / self.world_min_max
        ]
    ...
    def next_coords(self, actions, position, orientation):
        ...
        self.bot.rl_bot_ori[0] = self.bot.rl_bot_ori[0] if self.bot.rl_bot_ori[0] < np.pi else -(np.pi - self.bot.rl_bot_ori[0] % (np.pi))

        _, _, current_yaw = quat_to_euler_angles(orientation)
        world_theta = current_yaw + theta
        world_theta = world_theta if world_theta < np.pi else -(np.pi - world_theta % (np.pi))

        # print(f"RL_Bot_Ori: {np.rad2deg(self.bot.rl_bot_ori[0])}, Omni_Theta: {np.rad2deg(world_theta)}")
        ...
        return position+next_pos, self.orien, world_theta
    ...
    def step(self, action):  # action = [l, theta], l in [-0.5, 1] and theta in [-1, 1]
        ...
        self.next_pos, self.next_ori, world_theta = self.next_coords(
        ...
        wandb.log({"Bot_x": mlp_obs[0][0], "Bot_y": mlp_obs[0][1], "Goal_x": mlp_obs[1][0], "Goal_y": mlp_obs[1][1]})

        # Identify relevant target chunk
        min_dist = np.inf
        target_chunk = None
        for i in range(len(chunk)):
            dist = np.linalg.norm(chunk[i] - mlp_obs[1])
            if dist < min_dist:
                min_dist = dist
                target_chunk = i
        ...
        new_row = pd.DataFrame({
                                ...
                                "Ep_Timestep": self.timestep,
                                "Global_Timestep": self.global_timesteps,
                                "Bot_x": mlp_obs[0][0], 
                                "Bot_y": mlp_obs[0][1], 
                                "Goal_x": mlp_obs[1][0], 
                                "Goal_y": mlp_obs[1][1], 
                                "World_Theta": self.bot.rl_bot_ori[0], 
                                "Relative_Theta": mlp_obs[2],
                                "L": action[0], 
                                "Theta": action[1], 
                                "Xp": self.next_pos[0], 
                                "Yp": self.next_pos[1],
                                "Prev_Chunk_x": chunk[target_chunk-1][0] if target_chunk-1 > 0 else chunk[target_chunk][0],
                                "Prev_Chunk_y": chunk[target_chunk-1][1] if target_chunk-1 > 0 else chunk[target_chunk][0],
                                "Current_Chunk_x": chunk[target_chunk][0],
                                "Current_Chunk_y": chunk[target_chunk][1],
                                "Next_Chunk_x": chunk[target_chunk+1][0] if target_chunk+1 < len(chunk) else chunk[-1][0],
                                "Next_Chunk_y": chunk[target_chunk+1][1] if target_chunk+1 < len(chunk) else chunk[-1][1],
                                })
    
        new_row.to_csv(self.csv_file, mode='a', header=False, index=False)
        # self.obs_df = pd.concat([self.obs_df, new_row], ignore_index=True)
...
#             # log_data = {
#             #     "callback_global_timesteps": self.step_count,
...
def main():
    ...
    parser.add_argument(
        "--wandb_project",
        type=str,
        default=env_config.training.wandb_project_name,
    ...
    parser.add_argument(
        "--storage_path",
        type=str,
        default="/home/rahm/SN_LOGS",
        help="Choose the path to store the logs and ckpts",
    )
    parser.add_argument(
        "-g",
        "--gpu",
