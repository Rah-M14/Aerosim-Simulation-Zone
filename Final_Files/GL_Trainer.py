...
class SocEnv(gym.Env):
    ...
    def __init__(
        ...
        from Adv_SocRewards_RRT import SocialReward
        ...
        print("Bot, Navmesh and People initialised!")
        self.kit.update()
        ...
        # PATH MANAGER
        self.path_manager = PathManager(image_path="standalone_examples/api/omni.isaac.kit/WR_World.png", chunk_size=10)

        # RL PARAMETERS
        self.seed(seed)

        # mlp_zeros = np.concatenate([np.zeros(2), np.zeros(4), np.zeros(2), np.zeros(20), np.zeros(1)])
        mlp_zeros = np.zeros(29)
        ...
        self.observation_space = spaces.Dict(
            {
                "vector": spaces.Box(
                    low=-np.inf,
                    high=np.inf,
                    shape=(self.mlp_context_frame_length, 29),
                    dtype=np.float32,
                ),
                "image": spaces.Box(
                    low=0,
    ...
    def get_observations(self):
        ...
        if chunk.shape[0] != 10:
            ...
            pad_width=((0, 10 - chunk.shape[0]), (0, 0)),
            mode='edge'
        )
        ...
        mlp_states_un = [bot_world_pos[:2], bot_world_ori, goal_world_pos[:2], chunk_flat, timestep]

        if self.state_normalize == True:
            chunk_flat[0::2] = chunk_flat[0::2] / self.world_limits[0]
            chunk_flat[1::2] = chunk_flat[1::2] / self.world_limits[1]
    ...
    def de_normalize_states(self, observation):
        return [
            ...
            observation[3],
            observation[4] * self.max_episode_length,
        ]
    ...
    def next_coords(self, actions, position, orientation):
        ...
        l_noise = np.random.uniform(low=-3, high=3, size=1)
        l = l + l_noise[0]
    ...
    def step(
        ...
        reward, to_point_rew, reward_dict, ep_rew_dict = (
            self.reward_manager.compute_reward(
                prev_bot_pos[:2], mlp_obs[0], mlp_obs[-3], chunk, lidar_data
            )
        )
...
class CustomCombinedExtractor(BaseFeaturesExtractor):
    def __init__(
        ...
        self.vec_lstm = nn.LSTM(29, self.vec_lstm_out_size, num_layers=1)
        ...
        for key, subspace in self._observation_space.spaces.items():
            if key == "vector":
                if observations[key].shape[0] == 1:
                    ...
                    core_in = (
                        observations[key].view(self.mlp_context, -1, 29).to(model_device)
                    )
                    lstm_out = self.vec_lstm(core_in)[1][0]
