...
from stable_baselines3.common.callbacks import BaseCallback, CheckpointCallback, ProgressBarCallback, EvalCallback
...
class SocEnv(gym.Env):
    ...
    def __init__(
        ...
        usd_path="standalone_examples/api/omni.isaac.kit/Final_WR_World/New_Core.usd"
    ...
    def get_mlp_combined_context(self):
        return th.tensor(np.array(self.mlp_context_frame, dtype=np.float32, device=f'cuda{self.gpus[0]}'))

    def get_img_combined_context(self):
        return th.tensor(np.array(self.img_context_frame, dtype=np.float32, device=f'cuda{self.gpus[0]}'))
...
class BestModelCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
            ...
            if self.model.ep_info_buffer and len(self.model.ep_info_buffer) > 0:
                if mean_reward > self.best_mean_reward:
                    ...
                    if self.save_replay_buffer and hasattr(self.model, "replay_buffer") and self.model.replay_buffer is not None:
                        replay_buffer_path = os.path.join(self.save_path, f"best_SocNav_{self.algo}_Replay_Buffer.pkl")
        ...
        if self.n_calls % self.freq == 0:
            ...
            metadata_path = os.path.join(self.save_path, f"SocNav_{self.algo}_{self.n_calls}_ckpt_metadata.json")
            with open(metadata_path, 'w') as f:
                json.dump(metadata, f)
...
class CustomCombinedExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Dict, device, cnn_output_dim: int = 256, algo="ppo", mlp_context=32, img_context=16):
        ...
        self.vec_lstm = nn.LSTM(9, self.vec_lstm_out_size, num_layers=1).to(self.device)
        ...
        self.img_lstm = nn.LSTM(self.n_flatten_size, self.img_lstm_out_size, num_layers=2).to(self.device)
...
def create_model(algo: str, my_env, device, policy_kwargs: dict, tensor_log_dir: str):
    if algo.lower() == 'ppo':
        return PPO(
                PPOMultiPolicy,
                my_env,
...
def main():
    ...
    try:
        with wandb.init(project="Mod_Isaac_Soc_Nav", name=f"{args.algo.upper()}_training") as run:
            ...
            print(f"the gpus being used are : {gpus}")
            ...
            policy_kwargs = {
                'ppo' : dict(net_arch=[dict(pi=[256, 512, 512, 256], vf=[256, 512, 512, 256])], activation_fn=th.nn.Tanh,
                            ...
                            features_extractor_kwargs=dict(cnn_output_dim=256, algo="ppo", device=device, mlp_context=args.mlp_context, img_context=args.img_context)),
                'sac' : dict(net_arch=dict(pi=[256, 512, 512, 256], qf=[256, 512, 512, 256]), activation_fn=th.nn.ReLU, use_sde=False, log_std_init=-3,
                             features_extractor_class=CustomCombinedExtractor,
                             features_extractor_kwargs=dict(cnn_output_dim=256, algo="sac", device=device, mlp_context=args.mlp_context, img_context=args.img_context))}[args.algo.lower()]
