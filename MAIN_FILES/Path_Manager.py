...
class PathManager:
    ...
    def plan_new_path(self, start_pos, goal_pos):
        ...
        self.goal = goal_pos
        print(f"RRT* Path Planner called")
    ...
    def get_next_chunk(self, current_pos):
        ...
        # print(f"Remaining path shape: {remaining_path.shape}")
    ...
    #     elif self.current_index + self.chunk_size >= len(self.current_path):
    #         rem_path = self.current_path[self.current_index:]
    #         final_element = np.array(self.current_path[-1])
    #         padding = np.array([final_element] * (self.chunk_size - len(rem_path)))
    #         self.new_chunk = np.append(rem_path, padding)
        
    #     if self.new_chunk.shape[0] < self.chunk_size:
    #         padding = np.array([self.new_chunk[-1]] * (self.chunk_size - len(self.new_chunk)))
    #         self.new_chunk = np.append(self.new_chunk, padding)
    #     return np.array(self.new_chunk)
