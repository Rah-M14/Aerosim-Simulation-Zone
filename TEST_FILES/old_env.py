...
class PathFollowingEnv(gym.Env):    
    def __init__(self, image_path: str, algo: str, max_episode_steps: int = 1000, chunk_size: int = obs_config.chunk_size, headless: bool = False):
        super().__init__()
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[np.ndarray, Dict]:
        ...
        self.reward_manager.ep_log(self.episode_num)
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        obs = np.concatenate([
            ...
            np.array([self.current_step]) # Timestep (1)
        ]).astype(np.float32)
