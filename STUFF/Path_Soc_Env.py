...
class SocEnv(gym.Env):
    ...
    def __init__(
        ...
        state_normalize,
        logdir,
        world_map="standalone_examples/api/omni.isaac.kit/WR_World.png",
        mlp_context=120,
        img_context=30,
        skip_frame=1,
        physics_dt=1.0 / 60.0,
        ...
        from Path_Bot_Controller import RLBotController, RLBotAct
        ...
        self.controller = WheelBasePoseController(name='Path_Bot_Controller',
                                                    ...
                                                    is_holonomic=False)
        self.bot_act = RLBotAct(self.bot.rl_bot, self.controller, n_steps=10)
        ...
        # PATH PARAMETERS
        env_map = world_map
        self.planner = RRTStarPlanner(env_map)
        self.path = []
    ...
    def reset(self):
        ...
        for attempt in range(max_attempts):
            ...
            print(f"Attempt {attempt + 1}: Start pose: {self.bot.rl_bot.start_pose[0]}")
            print(f"Attempt {attempt + 1}: Goal Pose: {self.bot.rl_bot.goal_pose}")

            start_time = time.time()
            try:
                result = self.planner.plan(self.bot.rl_bot.start_pose[0][:2], self.bot.rl_bot.goal_pose[:2])
                if time.time() - start_time > timeout:
                    print(f"Attempt {attempt + 1}: Path planning timed out. Retrying...")
                    continue
                self.path = result['path']
                break
            except Exception as e:
                print(f"Attempt {attempt + 1}: Path planning failed with error: {e}. Retrying...")
        else:
            raise RuntimeError("Failed to find a valid path after multiple attempts")
    
        for point_pair in self.path:
    ...
    def step(self, action): # action = [linear_velocity, angular_velocity] both between [-1,1]
        ...
        if len(self.path) != 0:
            cur_pos, next_pos = self.path.pop(0)
            print(f"Current timestep: {self.timestep}")
            print(f"Moving from {prev_bot_pos[:2]} to ({next_pos[0]:.2f}, {next_pos[1]:.2f})")

            self.bot_act.move_bot(cur_pos=prev_bot_pos[:2], cur_ori=prev_bot_ori, next_pos=np.array(next_pos))
...
def main():
    ...
    parser.add_argument('--world_map', type=str, default="standalone_examples/api/omni.isaac.kit/WR_World.png", help='Choose the world map to train in')
    ...
    try:
        with wandb.init(project="Mod_Isaac_Soc_Nav", name=f"{args.algo.upper()}_training") as run:
            ...
            my_env = SocEnv(algo=args.algo, botname=args.botname, headless=args.headless, state_normalize=args.state_normalize, mlp_context=args.mlp_context, img_context=args.img_context, logdir=log_dir, world_map=args.world_map)
