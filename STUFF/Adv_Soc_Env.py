...
class SocEnv(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Dict({
            ...
            'image': spaces.Box(low=0, high=1, shape=(3, self.img_context_frame_length, 64, 64), dtype=np.float32),
    ...
    def get_img_combined_context(self):
        return th.swapaxes(th.tensor(np.array(self.img_context_frame, dtype=np.float32)), 0, 1)
        ...
        for key, subspace in observation_space.spaces.items():
            if algo == "ppo":
                if key == "vector":
                    ...
                    extractors[key]['lstm'] = nn.LSTM(13, 256, num_layers=2)
                    ...
                    extractors[key]['core'] = nn.Sequential(
                        nn.Conv3d(n_input_channels, 32, kernel_size=(5, 3, 3), stride=(2, 1, 1), padding=(2, 1, 1)),
                        nn.ReLU(),
                        nn.BatchNorm3d(32),
                        nn.Conv3d(32, 64, kernel_size=(5, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)),
                        nn.ReLU(),
                        nn.BatchNorm3d(64),
                        nn.Conv3d(64, 128, kernel_size=3, stride=(1, 1, 1), padding=(1, 1, 1)),
                        nn.ReLU(),
                        nn.BatchNorm3d(128),
                        nn.Conv3d(128, 256, kernel_size=3, stride=(1, 1, 1), padding=(1, 1, 1)),
                        nn.ReLU(),
                        nn.BatchNorm3d(256),
                ...
                if key == "vector":
                    ...
                    n_input_channels = 3
                    extractors[key]['core'] = nn.Sequential(
                        nn.Conv3d(n_input_channels, 32, kernel_size=(7, 5, 5), stride=(2, 1, 1), padding=(3, 2, 2)),
                        nn.ReLU(),
                        nn.BatchNorm3d(32),
                        nn.Conv3d(32, 64, kernel_size=(5, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)),
                        nn.ReLU(),
    ...
    def forward(self, observations) -> th.Tensor:
        ...
        for key, extractor in self.extractors.items():
            if key == "vector":
                ...
                if observations[key].shape[0] == 1: 
                    core_in = observations[key].view(-1, 3, self.img_context, 64, 64).to(self.device)
                    core_out = extractor['core'](core_in)
                    # print(f"img core out : {core_out.shape}")
                    encoded_tensor_list.append(core_out)
                else:
                    core_in = observations[key].view(-1, 3, self.img_context, 64, 64).to(self.device)
                    core_out = extractor['core'](core_in)
                    # print(f"img core out else: {core_out.shape}")
                    encoded_tensor_list.append(core_out)
...
def main():
    ...
    parser.add_argument('--mlp_context', type=int, default=60, help='Length of the MLP context')
    ...
    try:
        with wandb.init(project="Mod_Isaac_Soc_Nav", name=f"{args.algo.upper()}_training") as run:
            ...
            policy_kwargs = {
                'ppo' : dict(net_arch=[dict(pi=[256, 128, 256], vf=[256, 128, 256])], activation_fn=th.nn.Tanh,
                ...
                'sac' : dict(net_arch=dict(pi=[128, 64, 128, 256], qf=[128, 64, 128, 256]), activation_fn=th.nn.ReLU, use_sde=False, log_std_init=-3,
                             features_extractor_class=CustomCombinedExtractor,
