...
def plot_agent_behavior(binary_img, sensor, world_theta, action, lidar_dists, world_limits, return_fig=False):
    """
    Plot the agent behavior using the binary environment, sensor position, heading and LiDAR data.
    If return_fig is True, the function returns the matplotlib figure (to allow logging via WandB).
    Otherwise, the plot is shown for 2 seconds and then auto-closed.
    """
    # Create an image copy for plotting.
    img = binary_img.copy()
    
    # Convert sensor position to pixel coordinates.
    ...
    # -----------------------------
    # Compute the free space (safe) direction.
    # -----------------------------
    ...
    # Plot the free direction as an arrow.
    arrow_length = 50  # Adjust based on plotting scale.
...
class LocalPlannerCNN(nn.Module):
    def __init__(self, num_lidar=360, conv_channels=[16, 32, 64], fc_hidden=128):
        super(LocalPlannerCNN, self).__init__()
        self.conv1 = nn.Conv1d(1, conv_channels[0], kernel_size=5, padding=2, padding_mode='circular')
        self.bn1 = nn.BatchNorm1d(conv_channels[0])
        self.conv2 = nn.Conv1d(conv_channels[0], conv_channels[1], kernel_size=5, padding=2, padding_mode='circular')
        self.bn2 = nn.BatchNorm1d(conv_channels[1])
        self.conv3 = nn.Conv1d(conv_channels[1], conv_channels[2], kernel_size=5, padding=2, padding_mode='circular')
        self.bn3 = nn.BatchNorm1d(conv_channels[2])
        self.global_pool = nn.AdaptiveAvgPool1d(1)
        self.fc_theta = nn.Linear(2, 32)
...
# ---------------------
def train_local_planner(num_epochs=2000, batch_size=128, lr=1e-4,
                          safe_distance=1.0, alpha=2.0, sigma=0.2,
                          gamma=0.2, temperature=0.5, beta=20.0,  # Increased for stronger collision penalty.
                          delta=2.0,
                          image_path=r"F:\Aerosim-Simulation-Zone\Try\New_WR_World.png"):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    # Initialize WandB
    wandb.init(project="LocalPlanner", config={
    ...
    scaler = amp.GradScaler()  # Use default AMP GradScaler
    ...
    for epoch in pbar:
        for batch in dataloader:
            # Efficient conversion of batch items.
            ...
            with amp.autocast():
                ...
                predicted_heading = torch.atan2(torch.sin(world_theta_tensor + theta_pred_norm * np.pi),
                ...
                angle_error = torch.atan2(torch.sin(predicted_heading - best_heading),
        ...
        scheduler.step()
        pbar.set_description(f"Epoch {epoch} | Loss: {loss.item():.4f} | Mean L: {L.mean().item():.4f}")
        wandb.log({"epoch": epoch, "loss": loss.item(), "mean_speed": L.mean().item()}, step=epoch)
        
        # Every 100 epochs, log the agent behavior plot.
        if epoch % 10 == 0:
...
if __name__ == "__main__":
    trained_model = train_local_planner(num_epochs=2000, batch_size=128, lr=1e-4,
                                        safe_distance=1.0, alpha=2.0, sigma=0.2,
                                        gamma=0.2, temperature=0.5, beta=20.0, delta=2.0,
                                        image_path=r"F:\Aerosim-Simulation-Zone\Try\New_WR_World.png")
    trained_model.eval()
    torch.save(trained_model, "try_local_planner.pth")
    wandb.finish()