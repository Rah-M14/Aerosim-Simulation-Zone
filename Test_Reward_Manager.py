...
class SocNavManager:
    def __init__(self, components, method, log_dir):
        ...
        self.live_reward = self.reward_config.live_reward
        self.timeout_penalty = self.reward_config.timeout_penalty
        self.goal_reward = self.reward_config.goal_reward
        self.boundary_coll_penalty = self.reward_config.boundary_coll_penalty
        ...
        self.path_corridor_width = self.reward_config.path_corridor_width
        self.max_path_deviation = self.reward_config.max_path_deviation
        self.path_following_factor = self.reward_config.path_following_factor
        self.deviation_penalty_factor = self.reward_config.deviation_penalty_factor
    ...
    def compute_reward(self, prev_bot_pos, cur_bot_pos, goal_pos, chunk=None, lidar_data=None):
        ...
        path_reward = self.path_following_reward(cur_bot_pos, chunk)
        reward_dict['path_rew'] = path_reward
        ...
        self.current_rew = reward_dict['cosine_sim_rew'] + reward_dict['dist_to_goal_rew'] + reward_dict['smooth_crash_rew'] + reward_dict['path_rew'] + reward_dict['close_coll_rew'] + self.live_reward 
    ...
    def path_following_reward(self, cur_bot_pos, path_chunk):
        if path_chunk is None:
            return 0
        ...
        for i in range(len(path_chunk) - 1):
            p1 = path_chunk[i]
            p2 = path_chunk[i + 1]
            segment = p2 - p1
            ...
            t = np.dot(cur_bot_pos - p1, segment) / (len_segment * len_segment)
            t = max(0, t)
            projection = p1 + t * segment
            dist = np.linalg.norm(cur_bot_pos - projection)
            if dist < min_dist:
                min_dist = dist
        ...
        self.logger.info(f"Path following - Distance: {min_dist:.2f}, Reward: {reward:.2f}, Clipped Reward: {clipped_reward:.2f}")
    ...
    def update_ep_rewards(self, reward_dict):
        ...
        self.ep_reward_dict['ep_path_rew'] += reward_dict['path_rew']
